...
import { cartCount, decrement, deleteProduct, increment } from "../redux/CartSlice";
...
function Cart() {
    ...
    return (
        <div >
            ...
            {cart.map((item, index) =>
                <div key={index} className='grid grid-rows-1 grid-cols-7 grid-flow-row-dense m-1 h-20 rounded-md border-2 border-slate-800 p-1 items-center ' >
                    <div className="col-span-1  w-12 " > <img src={item.image} alt="" /> </div>
                    ...
                    <button className="flex justify-center rounded-full bg-red-600 p-2  m-auto" onClick={() => dispatch(deleteProduct(item.id))}>Remove</button>
